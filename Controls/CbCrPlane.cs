...
// 指定した輝度における Cb-Cr 色差平面を描いた画像を生成する。横軸が Cb(左 0→右 255)、縦軸が Cr(下 0→上 255)で、パッドの縦方向(上ほど大)と合わせるため上端を Cr=255 とする。各画素を指定形式の YCbCr→RGB へ変換し、RGB が 0–255 に収まる有効ガモットの内側はその色をそのまま塗る。収まらない外側の見せ方は GamutOutOfRangeStyle で切り替える。輝度・符号化形式(規格・レンジ)・色域外の見せ方が変わるたびに作り直す想定。色制限が有効なら有効ガモット内の色をその制限へ丸めてから並べる。
public static class CbCrPlane
{
	...
	// 指定した画素サイズで、与えた輝度・符号化形式・色制限設定・表示倍率・色域外の見せ方の Cb-Cr 色差平面を描いた WriteableBitmap を作る。全面を不透明で塗り、色域内は実色(色制限が有効ならその制限へ丸めた色)、色域外は style に従う。ハッチは DIP 指定の寸法を表示倍率で画素へ直し、L-C 平面・a-b 平面のハッチと太さ・間隔をそろえる。
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double luma, YCbCrFormat format, SnapSettings snap, double scale, GamutOutOfRangeStyle style)
	{
		...
		{
			double cbOffset = (255.0 * ((x + 0.5) / pixelWidth)) - 128.0;
		...
		{
			double crOffset = (255.0 * (1.0 - ((y + 0.5) / pixelHeight))) - 128.0;
	...
	private static (byte R, byte G, byte B) FillColor(bool fillClamped, SnapSettings snap, double lumaLinear, double rr, double gg, double bb)
	{
		if (!fillClamped)
		{
	...
	private static (byte R, byte G, byte B) SnapIf(SnapSettings snap, byte r, byte g, byte b)
	{
