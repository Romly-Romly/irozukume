...
// 指定した彩度における HSL の色相・輝度パッドの下地を描いた画像を生成する。横軸が色相(左 0 度→右 360 度)、縦軸が輝度(下 0=黒→上 1=白)で、彩度は引数で与える一定値とする。各画素を HSL→RGB へ変換し、色制限が有効なら設定の制限へ丸める。色相が横一面に渡って変わるため画像で賄う。彩度や色制限モードが変わるたびに作り直す想定。HueValuePlane の HSV を HSL へ替え、縦軸を明度から輝度へ替えた対の描画。
public static class HueLightnessPlane
{
	// 指定した画素サイズ・彩度・色制限設定で、色相・輝度パッドの下地を描いた WriteableBitmap を作る。全面を不透明で塗る。
