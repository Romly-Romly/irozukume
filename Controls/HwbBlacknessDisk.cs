...
// 角度=色相・中心からの半径=黒みの円盤の下地画像を生成する。各画素の中心からの角度を色相に、半径を黒み(中心 0→縁 1)に写し、白みは引数で与える一定値とする。角度の取り方は RingGeometry と揃えるため、つまみの位置と円盤の色がずれない。円の縁は不透明度を滑らかに落として背景になじませ、円の外は透明にする。白み+黒みが 1 を超える領域(白みが大きいときの外周側)は無彩色へ退化するが、HwbToRgb がその灰を返すため特別な処理は要らない。白みまたは色制限が変わったときだけ作り直す想定。HwbWhitenessDisk の半径を白みから黒みへ替えた対の描画。
public static class HwbBlacknessDisk
{
	...
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double whiteness, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。色制限の切替や白みドラッグで大きな円盤を塗り直す際に1スレッドでは重いため、全コアへ分散する。
		Parallel.For(0, pixelHeight, y =>
		{
			for (int x = 0; x < pixelWidth; x++)
			{
				...
				double blackness = Math.Clamp(radius / maxRadius, 0.0, 1.0);
