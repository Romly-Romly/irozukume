...
// 角度=色相・中心からの半径=明度の円盤の下地画像を生成する。各画素の中心からの角度を色相に、半径を明度(中心 0=黒→縁 1)に写し、彩度は引数で与える一定値とする。角度の取り方は RingGeometry と揃えるため、つまみの位置と円盤の色がずれない。円の縁は不透明度を滑らかに落として背景になじませ、円の外は透明にする。彩度が変わったとき、または色制限が変わったときだけ作り直す想定。HueSatDisk の半径を彩度から明度へ替えた対の描画。
	...
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double saturation, SnapSettings snap)
	{
		...
		Parallel.For(0, pixelHeight, y =>
		{
			for (int x = 0; x < pixelWidth; x++)
			{
				...
				double value = Math.Clamp(radius / maxRadius, 0.0, 1.0);
