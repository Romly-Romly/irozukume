...
// 角度=色相・中心からの半径=白みの円盤の下地画像を生成する。各画素の中心からの角度を色相に、半径を白み(中心 0→縁 1)に写し、黒みは引数で与える一定値とする。角度の取り方は RingGeometry と揃えるため、つまみの位置と円盤の色がずれない。円の縁は不透明度を滑らかに落として背景になじませ、円の外は透明にする。白み+黒みが 1 を超える領域(黒みが大きいときの外周側)は無彩色へ退化するが、HwbToRgb がその灰を返すため特別な処理は要らない。色相は円盤の全方位に現れるため色相を引数に取らず、黒みまたは色制限が変わったときだけ作り直す想定。HslHueSatDisk の HSL を HWB へ替え、半径=白み・固定成分=黒みにした対の描画。
public static class HwbWhitenessDisk
{
	...
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double blackness, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。色制限の切替や黒みドラッグで大きな円盤を塗り直す際に1スレッドでは重いため、全コアへ分散する。
		Parallel.For(0, pixelHeight, y =>
		{
			for (int x = 0; x < pixelWidth; x++)
			{
				...
				double whiteness = Math.Clamp(radius / maxRadius, 0.0, 1.0);
