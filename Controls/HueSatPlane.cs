...
// 指定した明度における HSV の色相・彩度パッドの下地を描いた画像を生成する。横軸が色相(左 0 度→右 360 度)、縦軸が彩度(下 0→上 1)で、明度は引数で与える一定値とする。各画素を HSV→RGB へ変換し、色制限が有効なら設定の制限へ丸める。色相が横一面に渡って変わるため画像で賄う。明度や色制限モードが変わるたびに作り直す想定。HueValuePlane の縦軸を明度から彩度へ替えた対の描画。
public static class HueSatPlane
{
	// 指定した画素サイズ・明度・色制限設定で、色相・彩度パッドの下地を描いた WriteableBitmap を作る。全面を不透明で塗る。
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double value, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。画素数が多いとき(大きなパッド)に1スレッドでは塗りきれず明度ドラッグでカクつくため、全コアへ分散する。
