...
// 角度=色相・中心からの半径=彩度の円盤(HSL ホイール)の下地画像を生成する。各画素の中心からの角度を色相に、半径を彩度(中心 0→縁 1)に写し、輝度は引数で与える一定値とする。角度の取り方は RingGeometry と揃えるため、つまみの位置と円盤の色がずれない。円の縁は不透明度を滑らかに落として背景になじませ、円の外は透明にする。色相は円盤の全方位に現れるため色相を引数に取らず、輝度または色制限が変わったときだけ作り直す想定。HueSatDisk の HSV を HSL へ替え、固定成分を明度から輝度へ替えた対の描画。
public static class HslHueSatDisk
{
	...
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double lightness, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。色制限の切替や輝度ドラッグで大きな円盤を塗り直す際に1スレッドでは重いため、全コアへ分散する。
