...
// 指定した輝度における HSL の色相・彩度パッドの下地を描いた画像を生成する。横軸が色相(左 0 度→右 360 度)、縦軸が彩度(下 0→上 1)で、輝度は引数で与える一定値とする。各画素を HSL→RGB へ変換し、色制限が有効なら設定の制限へ丸める。色相が横一面に渡って変わるため画像で賄う。輝度や色制限モードが変わるたびに作り直す想定。HueLightnessPlane の縦軸を輝度から彩度へ替えた対の描画。
public static class HslHueSatPlane
{
	// 指定した画素サイズ・輝度・色制限設定で、色相・彩度パッドの下地を描いた WriteableBitmap を作る。全面を不透明で塗る。
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double lightness, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。画素数が多いとき(大きなパッド)に1スレッドでは塗りきれず輝度ドラッグでカクつくため、全コアへ分散する。
		Parallel.For(0, pixelHeight, y =>
		{
			// 上端を彩度 1、下端を彩度 0 とし、パッドの縦方向(上ほど大)に合わせる。
			double saturation = 1.0 - ((y + 0.5) / pixelHeight);
