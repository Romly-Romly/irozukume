...
// 角度=色相・中心からの半径=輝度の円盤の下地画像を生成する。各画素の中心からの角度を色相に、半径を輝度(中心 0=黒→縁 1=白)に写し、彩度は引数で与える一定値とする。角度の取り方は RingGeometry と揃えるため、つまみの位置と円盤の色がずれない。円の縁は不透明度を滑らかに落として背景になじませ、円の外は透明にする。彩度が変わったとき、または色制限が変わったときだけ作り直す想定。HslHueSatDisk の半径を彩度から輝度へ替えた対の描画。
public static class HueLightnessDisk
{
	...
	public static WriteableBitmap Create(int pixelWidth, int pixelHeight, double saturation, SnapSettings snap)
	{
		...
		// 各行は互いに素な画素範囲へ書き込むため、行単位で並列化してよい。色制限の切替や彩度ドラッグで大きな円盤を塗り直す際に1スレッドでは重いため、全コアへ分散する。
		Parallel.For(0, pixelHeight, y =>
		{
			for (int x = 0; x < pixelWidth; x++)
			{
				...
				double lightness = Math.Clamp(radius / maxRadius, 0.0, 1.0);
